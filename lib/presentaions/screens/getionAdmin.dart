...
class _AdminManagementPageState extends State<AdminManagementPage> {
  ...
  Future<void> fetchUsers() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Filter the users with emails ending with 'Trustify@contact.tn'
        final filteredUsers = allUsers.where((user) {
          final email = user['email'] as String?; // Safely access the email field
          return email != null && email.endsWith('Trustify@contact.tn');
        }).toList();

        setState(() {
          _users = filteredUsers;
          _filteredUsers = filteredUsers; // Initially, show all filtered users
        });
      } else {
  ...
  void _showEditUserDialog(String userId, String email, String companyName, String password) {
    TextEditingController emailController = TextEditingController(text: email);
    TextEditingController companyNameController = TextEditingController(text: companyName);
    TextEditingController registrationController = TextEditingController(text: password);
  ...
  Future<void> _updateUser(String userId, String email, String password) async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      var token = prefs.getString('accessToken');
                    ...
                    children: [
                      TextFormField(
                        ...
                        validator: (value) {
                          ...
                          RegExp emailPattern = RegExp(r'.+Trustify@contact\.tn$');
  ...
  Future<void> _addUser(
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        throw Exception('Failed to add admin: ${response.statusCode}');
      }
    } catch (error) {
      ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    trailing: Row(
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () {
                            _showEditUserDialog(userId, email, companyName, password);
                          },
                        ),
                        IconButton(
